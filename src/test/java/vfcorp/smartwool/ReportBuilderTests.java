...
public class ReportBuilderTests {
	...
	public void testPaymentsReportBuilder() {
		...
		try {
			...
			Mockito.when(client.businessLocations()).thenReturn(businessLocationsAdapter);
			Mockito.when(client.businessLocations().retrieve()).thenReturn(merchantResult);
			...
			HashMap<String, List<Payment>> report = new PaymentsReportBuilder("connect.squareupstaging.com", "test", "test", client)
				.forLocations(Arrays.asList(locations))
				.build();
	...
	public void testRefundsReportBuilder() {
		...
		try {
			...
			HashMap<String, List<Refund>> report = new RefundsReportBuilder("connect.squareupstaging.com", "test", "test", client)
				.forLocations(Arrays.asList(locations))
				.build();
