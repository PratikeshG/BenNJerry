...
public class TlogTest {
    ...
    @Test
    public void parse_AuthorizationCodeGeneratedWithExternalEmployeeIDs_EmployeeIDsAreExternalEmployeeIDs()
            throws Exception {
        ...
        List<String> authorizationCodeStrings = tlogStrings.stream()
                .filter(s -> s.startsWith(AUTHORIZATION_CODE) == true).collect(Collectors.toList());

        assertEquals("expecting only one authorization code", 1, authorizationCodeStrings.size());
        assertEquals("authorization code should be of a specific format", "023        180externalId",
                authorizationCodeStrings.get(0));
    }
    /*
    @Test
