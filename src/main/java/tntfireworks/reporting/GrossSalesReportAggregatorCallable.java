...
public class GrossSalesReportAggregatorCallable extends TntReportAggregator implements Callable {
    private static Logger logger = LoggerFactory.getLogger(GrossSalesReportAggregatorCallable.class);
    ...
    public Object onCall(MuleEventContext eventContext) throws Exception {
        ...
        logger.info("Start gross sales report generation");

        // build report from payloads
        List<List<GrossSalesPayload>> payloadAggregate = (List<List<GrossSalesPayload>>) message.getPayload();
        ...
        // add file rows
        for (List<GrossSalesPayload> masterPayload : payloadAggregate) {
            for (GrossSalesPayload grossPayload : masterPayload) {
                reportBuilder.append(grossPayload.getRow());
            }
        }

        // generate report into file
        String reportName = fileDate + "-report-9.csv";
    ...
    private boolean payloadExists(List<List<GrossSalesPayload>> payloadAggregate) {
